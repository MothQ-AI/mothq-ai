...
const releases = [["MothQ", "Edge-native reasoning architecture", "In progress"], ["Local runtime", "Inference tools and deployment", "In progress"], ["Technical reports", "Architecture and evaluation", "In progress"]];
  ...
  return (
    <main>
      ...
      <section className="runtime-wrap" id="runtime"><div className="runtime page-width">
        ...
        <div className="architecture" aria-label="MothQ target architecture"><p className="mono-label">Target: laptops to local devices</p><div className="architecture-flow"><div className="flow-column"><span>User input</span><span>Local context</span><span>Tool use</span><span>On-device data</span></div><div className="model-node"><strong>MothQ</strong><span>Reasoning model</span><small>In development</small></div><div className="flow-column output"><span>Reasoning</span><span>Planning</span><span>Tool calling</span><span>Local execution</span></div></div><div className="runtime-facts"><span>Edge-native</span><span>Open source</span><span>Local-first</span></div></div>
      </div></section>

      <section className="models page-width" id="models"><div className="section-rule"><span className="mono-label">Models in development</span></div><div className="model-grid"><div className="model-heading"><h2>MothQ</h2><p>No public model yet.</p></div><p className="model-copy">We are building an edge-native reasoning model for local deployment. Open weights, practical inference, and a smaller environmental footprint remain our direction.</p><div className="model-specs"><span>Status <strong>In development</strong></span><span>Direction <strong>Open</strong></span></div></div><div className="model-links"><a href="#research"><b>Our approach</b><span>Architecture, goals, and evaluation direction</span></a><a href="#community"><b>Follow updates</b><span>News from MothQ as work progresses</span></a><a href="#community"><b>Build with us</b><span>Open research needs open collaborators</span></a></div></section>

      <section className="research page-width" id="research"><div className="section-rule"><span className="mono-label">Research, in progress</span><p>Compact reasoning for a more capable world.</p></div><div className="research-grid"><div><h3>Research</h3>{research.map(([title, type]) => <a href="#community" className="list-row" key={title}><span>{title}</span><small>{type}</small></a>)}</div><div><h3>Development</h3>{releases.map(([title, description, action]) => <a href="#models" className="list-row" key={title}><span>{title}<small>{description}</small></span><small>{action}</small></a>)}</div></div></section>
